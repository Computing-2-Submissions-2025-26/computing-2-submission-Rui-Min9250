...
describe("Cat Cake Kitchen game module", () => {
  describe("buildCakeQueue", () => {
    it("uses the level ingredient count to trim each cake recipe", () => {
      ...
      assert.deepEqual(queue[0].ingredients, ["strawberry", "cake flour", "egg"]);
    });
  });
  ...
  describe("collectIngredients", () => {
    it("collects an ingredient from the cat's current cell", () => {
      const state = {
        ...
        ingredientsOnGrid: new Map([[CatCakeKitchen.keyFor(0, 0), "strawberry"]]),
      };
      const collected = CatCakeKitchen.collectIngredients(state);

      assert.equal(collected.collected.has("strawberry"), true);
      assert.equal(collected.ingredientsOnGrid.has(CatCakeKitchen.keyFor(0, 0)), false);
    });
  ...
  describe("canReachAllIngredients", () => {
    it("reports reachable ingredients as reachable", () => {
      const state = {
        ...CatCakeKitchen.createGame(testLevel, heizi, cakes),
        ingredientsOnGrid: new Map([[CatCakeKitchen.keyFor(3, 3), "strawberry"]]),
      };
  ...
  describe("isCakeComplete", () => {
    it("returns true only when every ingredient in the recipe has been collected", () => {
      const cake = cakes[0];

      assert.equal(CatCakeKitchen.isCakeComplete(cake, new Set(["strawberry", "cake flour"])), false);
      assert.equal(CatCakeKitchen.isCakeComplete(cake, new Set(["strawberry", "cake flour", "egg", "cream"])), true);
    });
  });

  describe("calculateStars", () => {
    it("calculates stars from the fraction of time remaining", () => {
      assert.equal(CatCakeKitchen.calculateStars(testLevel, heizi, 25), "★★★");
      assert.equal(CatCakeKitchen.calculateStars(testLevel, heizi, 12), "★★☆");
      assert.equal(CatCakeKitchen.calculateStars(testLevel, heizi, 5), "★☆☆");
      assert.equal(CatCakeKitchen.calculateStars(testLevel, heizi, 0), "☆☆☆");
    });
  });
});
